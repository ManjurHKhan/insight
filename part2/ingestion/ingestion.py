...
while True:
    try:
        ...
        if is_get_request(msg):
            # print("I am here inside get reuqest")
            # Parse GET request for relevant information
            day, status, source = parse_log(msg)
            # print("passed parse_log", day, status, source)
            # Store in RabbitMQ
            local = 'y' if source == 'local' else 'n'
            body = json.dumps({'day': str(day), 'status': status, 'local': local})
